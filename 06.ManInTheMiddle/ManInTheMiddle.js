...
const KEYPHRASE = 'OrchidOneKangarooIsExpress';
...
socket.on('data', function(data) {
	...
	var str = data.toString().substring(15, data.toString().length);

	if (data[0] == 'SERVER->CLIENT:hello!') {
	    dh = crypto.createDiffieHellman(256);
	    dh.generateKeys();
	    var tempString = util.format('key|%s|%s\n', dh.getPrime('hex'), dh.getPublicKey('hex'));
	    socket.write(str);
	    socket.write(tempString);
		...
		socket.write(str.replace(',', '|')); // Don't know why it puts a comma, so I replace it back
